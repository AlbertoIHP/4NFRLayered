<li class="{{ Request::is('nfrs*') ? 'active' : '' }}">
    <a href="{!! route('nfrs.index') !!}"><i class="fa fa-edit"></i><span>Nfrs</span></a>
</li>

<li class="{{ Request::is('relevances*') ? 'active' : '' }}">
    <a href="{!! route('relevances.index') !!}"><i class="fa fa-edit"></i><span>Relevances</span></a>
</li>

<li class="{{ Request::is('professions*') ? 'active' : '' }}">
    <a href="{!! route('professions.index') !!}"><i class="fa fa-edit"></i><span>Professions</span></a>
</li>

<li class="{{ Request::is('areas*') ? 'active' : '' }}">
    <a href="{!! route('areas.index') !!}"><i class="fa fa-edit"></i><span>Areas</span></a>
</li>

<li class="{{ Request::is('roles*') ? 'active' : '' }}">
    <a href="{!! route('roles.index') !!}"><i class="fa fa-edit"></i><span>Roles</span></a>
</li>

<li class="{{ Request::is('functionalities*') ? 'active' : '' }}">
    <a href="{!! route('functionalities.index') !!}"><i class="fa fa-edit"></i><span>Functionalities</span></a>
</li>

<li class="{{ Request::is('permises*') ? 'active' : '' }}">
    <a href="{!! route('permises.index') !!}"><i class="fa fa-edit"></i><span>Permises</span></a>
</li>

<li class="{{ Request::is('users*') ? 'active' : '' }}">
    <a href="{!! route('users.index') !!}"><i class="fa fa-edit"></i><span>Users</span></a>
</li>

<li class="{{ Request::is('projects*') ? 'active' : '' }}">
    <a href="{!! route('projects.index') !!}"><i class="fa fa-edit"></i><span>Projects</span></a>
</li>

<li class="{{ Request::is('stakeholders*') ? 'active' : '' }}">
    <a href="{!! route('stakeholders.index') !!}"><i class="fa fa-edit"></i><span>Stakeholders</span></a>
</li>

<li class="{{ Request::is('goals*') ? 'active' : '' }}">
    <a href="{!! route('goals.index') !!}"><i class="fa fa-edit"></i><span>Goals</span></a>
</li>

<li class="{{ Request::is('softgoals*') ? 'active' : '' }}">
    <a href="{!! route('softgoals.index') !!}"><i class="fa fa-edit"></i><span>Softgoals</span></a>
</li>

<li class="{{ Request::is('softgoalNfrs*') ? 'active' : '' }}">
    <a href="{!! route('softgoalNfrs.index') !!}"><i class="fa fa-edit"></i><span>Softgoal Nfrs</span></a>
</li>

<li class="{{ Request::is('categories*') ? 'active' : '' }}">
    <a href="{!! route('categories.index') !!}"><i class="fa fa-edit"></i><span>Categories</span></a>
</li>

<li class="{{ Request::is('nfrs*') ? 'active' : '' }}">
    <a href="{!! route('nfrs.index') !!}"><i class="fa fa-edit"></i><span>Nfrs</span></a>
</li>

<li class="{{ Request::is('projects*') ? 'active' : '' }}">
    <a href="{!! route('projects.index') !!}"><i class="fa fa-edit"></i><span>Projects</span></a>
</li>

<li class="{{ Request::is('collaborators*') ? 'active' : '' }}">
    <a href="{!! route('collaborators.index') !!}"><i class="fa fa-edit"></i><span>Collaborators</span></a>
</li>

